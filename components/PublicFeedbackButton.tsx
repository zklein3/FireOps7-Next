...
import { submitPublicHoseTestingFeedback } from '@/app/actions/hose-testing'

export default function PublicFeedbackButton({ slug }: { slug: string }) {
  ...
  async function handleSubmit(formData: FormData) {
    setLoading(true)
    setError(null)
    const result = await submitPublicHoseTestingFeedback(slug, formData)
    if (result?.error) {
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-5 right-5 z-[9998] flex items-center gap-2 rounded-full bg-red-800 px-4 py-3 text-sm font-semibold text-white shadow-lg hover:bg-red-900 transition-colors"
      >
        💬 Feedback
      </button>
      {modal}
