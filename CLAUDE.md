...
### Granular Permission-Group System — Phase 1 + Phase 2a (dept-admin pages) done, on a local branch, NOT merged to main (2026-08-09)
...
**Where things actually stand — read this before assuming anything is live:**
- **Branch `main`** (pushed to `origin/main`, deployed to Vercel): Phase 1 only — schema (`department_permission_groups`, `department_personnel.permission_group_id`), the resolver (`lib/permissions.ts`), catalog (`lib/permission-catalog.ts`, 34 keys across 10 categories), group CRUD (`app/actions/permissions.ts`), the `/dept-admin/permission-groups` builder UI, and the group picker on both Add Personnel forms. **None of this is wired into any real authorization gate on `main`** — `system_role` is still fully authoritative for every existing check. (Note: this landed on `main` by accident — a same-day bug-fix push dragged along a commit that was meant to stay local until approved; low-risk since it doesn't touch any gate, left in place with the user's OK. See `feedback_local_only_branching` memory — local feature work now always goes on its own branch to prevent a repeat.)
- **Branch `feature/permission-groups-phase2`** (local only, NOT merged/pushed): wires `hasPermission()` into all 17 `/dept-admin/**/page.tsx` entry-redirect guards — the first real gate the permission-group checkboxes actually control. Added 8 new catalog keys to cover pages that had no existing match (`access_dept_admin_hub`, `manage_permission_groups`, `manage_kiosk_devices`, `manage_dept_setup`, `manage_police_settings`, `manage_fuel_storage`, `manage_inspection_settings`, `manage_medical_supply_setup`). Also fixed an inconsistency where 7 of the 17 pages had no sys-admin bypass while the other 10 did — all 17 are now consistent. `app/actions/permissions.ts`'s own gate migrated to the same resolver for consistency. Internal component-level admin/officer distinctions inside `events`/`training` pages were deliberately left untouched (out of scope for this pass). Build clean; verified live via Playwright against Winslow Fire Department (legacy admin retains access to all 17 pages, legacy member correctly denied). Full plan/reasoning: see prior session's plan file if still present, or `git log` on this branch.
- Also on `main` (separate, already pushed): `setFuelStorageModule` was missing the `isSysAdmin` bypass every sibling admin-settings function has — fixed as its own bug-fix commit, unrelated to whether Phase 2 ships.

**Phase 2b+ sequencing (future session, not started):** action files by risk — `accountability.ts` (~14 checks, has guest-token actors), `public-site.ts` (~11 checks, has public-facing anonymous forms), `ics.ts` (~8 checks), `departments.ts` (~9 checks beyond the bug fix above), `app/actions/users.ts` — then `layout.tsx` nav conditionals (`isOfficerOrAbove`/`isDeptAdmin`/`viewingSysAdminOverview`) last. A full-codebase audit this session found the real scope is larger than earlier estimated: ~199 raw `systemRole`/`isSysAdmin` references across ~94 files app-wide (not just the ~80–120/~130 originally guessed), including ~31 more `isOfficerOrAbove`-shaped inline checks on non-dept-admin dashboard pages (`/officer`, `/events`, `/equipment`, `/reports/*`, `/iso/*`, etc.) not yet touched by any phase.

**Known limitation, not yet fixed:** since `access_dept_admin_hub` is its own key, a department could grant hub access without granting any individual section's key — the person would see every `HubCard` but bounce off most of them on click, since each still gates on its own specific key. Card-level visibility filtering is reasonable future polish, not done.

**To resume:** `git checkout feature/permission-groups-phase2`, confirm `git log --oneline -3` shows the Phase 2 commit on top of Phase 1, `npm run build`, then continue with Phase 2b per the sequencing above. Do not merge to `main` without the user's explicit go-ahead — this feature is deliberately held local until production-ready.
