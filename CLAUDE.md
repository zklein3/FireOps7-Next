...
### Granular Permission-Group System — MIGRATION COMPLETE, on a local branch, NOT merged to main (2026-08-10)
...
- **Branch `feature/permission-groups-phase2`** (local only, NOT merged/pushed) — every commit build-verified clean and spot-checked live against Winslow Fire Department after each file:
  ...
  - **`REFERENCE.md`'s "Permission Model" section replaced** with a full per-key reference table (grouped by category, one row per key, listing exactly which pages/actions each grants) — built from a full-codebase grep of every `hasPermission()`/`hasPermissionForDepartment()` call site, not from memory. This stays on the feature branch (not pushed to `main`) since it documents branch-only code, unlike this CLAUDE.md section which is safe as session notes regardless of merge status.

**Every real `system_role`-based authorization gate in `app/` is now resolver-backed.** No known gaps remain — the hub-card-filtering and dead-key issues noted in earlier revisions of this section are both fixed.

**To resume / merge:** `git checkout feature/permission-groups-phase2`, confirm `git log --oneline -13` shows all 10 Phase 2 commits on top of Phase 1, `npm run build`. The migration itself is done — what's left is entirely the user's call on when to merge to `main`. Do not merge without the user's explicit go-ahead; this feature is deliberately held local until production-ready.
