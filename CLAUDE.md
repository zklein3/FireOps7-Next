...
### Granular Permission-Group System — Phase 1 + full Phase 2 (essentially every gate in app/) done, on a local branch, NOT merged to main (2026-08-10)
...
- **Branch `main`** (pushed to `origin/main`, deployed to Vercel): Phase 1 only — schema, resolver (`lib/permissions.ts`), catalog, group CRUD, the `/dept-admin/permission-groups` builder UI, group picker on Add Personnel forms. **None of this is wired into any real authorization gate on `main`** — `system_role` is still fully authoritative there. (This landed on `main` by accident — a same-day bug-fix push dragged along a commit meant to stay local; low-risk since it touched no gate, left in place with the user's OK. See `feedback_local_only_branching` memory — local feature work now always goes on its own branch.)
- **Branch `feature/permission-groups-phase2`** (local only, NOT merged/pushed) — every commit build-verified clean and spot-checked live against Winslow Fire Department after each file:
  ...
  - Files/spots deliberately left untouched throughout, all previously-noted internal component-level UI distinctions or platform-level sys-admin actions: `equipment/storage/page.tsx`'s cosmetic zero-qty-display `isAdmin`, `personnel/[id]/page.tsx`'s props passed to its client, `dept-admin/training`/`dept-admin/events`' internal `isAdmin` flags, and `layout.tsx` nav conditionals (`isOfficerOrAbove`/`isDeptAdmin`/`viewingSysAdminOverview` — different shape, drives UI/badges not access control, still not started).

**Known limitation, not yet fixed:** since `access_dept_admin_hub`/`access_officer_hub` are their own keys, a department could grant hub access without granting any individual section's key — the person would see every `HubCard` but bounce off most of them on click. Card-level visibility filtering is reasonable future polish, not done.

**To resume:** `git checkout feature/permission-groups-phase2`, confirm `git log --oneline -11` shows all 8 Phase 2 commits on top of Phase 1, `npm run build`, then pick up `layout.tsx` nav conditionals — the last remaining piece, deliberately saved for last since it's UI/badge-shaped rather than access-control-shaped. Do not merge to `main` without the user's explicit go-ahead — this feature is deliberately held local until production-ready.
