...
Gap found by the user while testing: dept admins had no way to reset a member's password themselves — only sys admin could, via `/admin/users` → "Force Reset" (`sysAdminForcePasswordReset`). Added `deptAdminForcePasswordReset(personnelId)` in `app/actions/users.ts`, same reset-to-`Hello1!` + `signup_status: 'temp_password'` mechanism, but gated to the caller's own admin role (`ctx.systemRole === 'admin' || ctx.isSysAdmin`) **and** an explicit check that the target actually has an active `department_personnel` row in the caller's own department — a multi-dept admin can't reset a password for someone who only belongs to a different department they don't administer. UI: "Reset Password" button next to "Edit Profile" on every card in `/dept-admin/personnel` (`DeptPersonnelClient.tsx`), inline amber confirm banner (same convention as every other destructive-ish confirm in this app), green success banner naming the new temp password on completion. Build verified clean; this is the only thing actually pushed to `origin/main` today — see below for what's still local-only.

### Granular Permission-Group System — Phase 1 built + verified, **not yet committed** (2026-08-07)

Replacing the fixed 3-tier `department_personnel.system_role` (`admin`/`officer`/`member`) with a granular, boolean, department-customizable permission model — a department defines named roles (e.g. "Chief") with individual checkboxes grouped by category (System Administration → renamed **Department Administration** in the catalog to avoid colliding with the real sys-admin role, Fleet, Equipment, Personnel, Training, etc.), same shape as a competitor's editor the user referenced. Full design audit + plan captured this session; see the plan doc for the complete schema/resolver/file list if resuming mid-build.

**Key decisions locked in:**
- **Sys admin (`personnel.is_sys_admin`) has zero involvement.** No new `/admin/*` route, no sys-admin-authored table. The starter role set ("Chief"/"Officer"/"Firefighter") is a **hardcoded TypeScript constant** (`DEFAULT_PERMISSION_TEMPLATES` in `lib/permission-catalog.ts`), same idiom as `VEHICLE_CHECK_DEFAULTS`/`DEFAULT_ACCOUNTABILITY_LANES` — lazily seeded into each department's own `department_permission_groups` table on first visit to `/dept-admin/permission-groups`, then the department customizes its own copy independently (rename, re-check, add groups, "Reset to default" re-pulls the current hardcoded constant). Sys admin stays exactly what it's always been — the platform/developer-level bypass, `ctx.isSysAdmin` short-circuiting every check to `true`, same as it bypasses every other department-scoped gate today.
- **A fully-checked group = "Full Admin" purely via booleans** — no separate hardcoded admin concept needed at the department level; once Phase 2 migrates real gates, a fully-checked group becomes functionally equivalent to today's `system_role: 'admin'`.
- **Phased rollout, not big-bang.** Phase 1 (this build) stands up the schema (`department_permission_groups` table + nullable `department_personnel.permission_group_id`), the resolver chokepoint (`lib/permissions.ts` → `hasPermission()`/`getPermissionSnapshot()`, with a `legacyMinRole`-driven fallback so anyone with no group assigned behaves identically to today), and the dept-admin builder UI (`/dept-admin/permission-groups`) — but does **not** touch any of the ~80–120 existing gate call-sites across ~130 files (confirmed via full-codebase audit: no shared permission helper exists today, at least 5 independently-written "officer-or-admin" implementations). Real authorization keeps running on `system_role` unchanged until Phase 2+ migrates each site.
- **Phase 2+ sequencing** (future session): canary the 5 duplicate officer-or-admin implementations first → dept-admin page guards (~16 files, uniform pattern) → action files by risk (`accountability.ts` 14 sites, `public-site.ts` ~15 sites, `ics.ts` 7 sites, `departments.ts` 6 sites — also fix a real pre-existing bug found during the audit, `setFuelStorageModule` missing its `isSysAdmin` bypass) → `layout.tsx` nav last (highest coupling, lowest security stakes).

**Phase 1 shipped this session — schema, resolver, and dept-admin UI, all verified against the live Winslow Fire Department (build clean, no type errors):**
- `lib/permission-catalog.ts` — 34-key `PERMISSION_CATALOG` across 10 categories (Department Administration, Personnel, Fleet, Equipment, Training, Events/Attendance, Operations/Incidents, Accountability/ICS, ISO, Medical, Public Site/Inbox) + hardcoded `DEFAULT_PERMISSION_TEMPLATES` (Chief/Officer/Firefighter)
- `lib/permissions.ts` — `getPermissionSnapshot()`/`hasPermission()` resolver, sys-admin bypass, legacy `system_role` fallback per-key
- `app/actions/permissions.ts` — full group CRUD, `ensurePermissionGroups` lazy-seed, `resetPermissionGroupToTemplate`, `updatePersonnelPermissionGroup`
- `department_permission_groups` table + `department_personnel.permission_group_id` (nullable FK) — migration applied
- `/dept-admin/permission-groups` (`PermissionGroupsClient.tsx` + shared `components/PermissionChecklist.tsx`) — new HubCard on `/dept-admin`
- Permission-group picker added to both Add Personnel forms (`app/(dashboard)/personnel/PersonnelAddForm.tsx`, `app/(dashboard)/dept-admin/personnel/DeptPersonnelClient.tsx`) and the existing-roster inline picker on `DeptPersonnelClient.tsx`; `createDeptMember` persists it
- **Verified live via Playwright + direct DB checks against Winslow Fire Department**: groups lazy-seed correctly on first visit (Chief/Officer/Firefighter), checkbox edits persist through a real reload, Reset to Default correctly re-pulls the current hardcoded template, a new from-scratch group creates correctly (`source_template_key: null`), and person-assignment writes `permission_group_id` correctly (tested and reverted against `member.winfire@fireops7.com`). Test artifacts cleaned up afterward — no residue left on the real department.
- Hit one unrelated environment issue mid-verification: a stale Turbopack dev-route manifest made every `/dept-admin/*` sub-route 404 (including untouched ones) until a clean dev server restart — not a code bug, just a dev-server quirk from adding a new route file while the server was already running.

**Commit status: everything above is on disk and build-verified, but not yet committed or pushed** — `git status` shows it all as modified/untracked on top of `706a2ed` (the password-reset commit, which *is* pushed — see above). Resume by reviewing `git diff` / `git status`, then commit when ready; nothing here has been lost, just deliberately held back so the password-reset fix could ship on its own first.

**Not yet built (Phase 2+):** none of the ~80–120 real gate call-sites are migrated yet — `system_role` is still fully authoritative everywhere. See the sequencing above.
