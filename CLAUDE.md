...
### Granular Permission-Group System — Phase 1 + all of Phase 2 (pages + all 4 action files) done, on a local branch, NOT merged to main (2026-08-10)
...
**Explicitly deferred (not started):** `app/actions/users.ts`, `layout.tsx` nav conditionals (`isOfficerOrAbove`/`isDeptAdmin`/`viewingSysAdminOverview`), and ~31 more `isOfficerOrAbove`-shaped inline checks on non-dept-admin dashboard pages (`/officer`, `/events`, `/equipment`, `/reports/*`, `/iso/*`, etc.) found during this session's full-codebase audit (~199 raw `systemRole`/`isSysAdmin` references across ~94 files app-wide, larger than originally estimated).

**Known limitation, not yet fixed:** since `access_dept_admin_hub` is its own key, a department could grant hub access without granting any individual section's key — the person would see every `HubCard` but bounce off most of them on click. Card-level visibility filtering is reasonable future polish, not done.

**To resume:** `git checkout feature/permission-groups-phase2`, confirm `git log --oneline -8` shows all 5 Phase 2 commits (dept-admin pages, departments.ts, ics.ts, accountability.ts, public-site.ts) on top of Phase 1, `npm run build`, then pick up `app/actions/users.ts` or `layout.tsx` nav next. Do not merge to `main` without the user's explicit go-ahead — this feature is deliberately held local until production-ready.
