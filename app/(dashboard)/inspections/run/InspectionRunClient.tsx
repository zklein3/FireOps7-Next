...
interface Step {
  ...
  linked_asset_options: { id: string; asset_tag: string; serial_number: string | null }[]
}
...
export default function InspectionRunClient({
  ...
  // Presence responses for quantity-only items
  const [presenceResponses, setPresenceResponses] = useState<Record<string, PresenceResponse>>({})

  // For asset-tracked items: which asset the user has selected per item slot
  // key = location_standard_id, value = asset_id selected
  const [selectedAssets, setSelectedAssets] = useState<Record<string, string>>({})

  // Which template selected per selected asset (if item has multiple templates)
  const [selectedTemplates, setSelectedTemplates] = useState<Record<string, string>>({})

  // Step responses: assetId -> stepId -> StepResponse
  ...
  function setStepResponse(asset_id: string, step_id: string, field: keyof StepResponse, value: any) {
  ...
  function getSelectedAsset(item: ChecklistItem): Asset | null {
    const assetId = selectedAssets[item.location_standard_id]
  ...
  function isComplete(): boolean {
    for (const item of checklistItems) {
      if (!item.requires_inspection) {
        ...
        // Must have selected an asset
        const asset = getSelectedAsset(item)
    ...
    <div className="max-w-lg">
      ...
      <div className="flex flex-col gap-4">
        {checklistItems.map(item => {
          const selectedAsset = getSelectedAsset(item)
          const template = selectedAsset ? getTemplate(item, selectedAsset.id) : null
            ...
            <div key={item.location_standard_id} className="rounded-xl bg-white shadow-sm border border-zinc-200 overflow-hidden">
              ...
              <div className="px-5 py-3 bg-zinc-50 border-b border-zinc-200 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-zinc-400">
                    {item.requires_inspection ? 'Asset inspection' : `Presence check · Qty ${item.expected_quantity}`}
                  </p>
                </div>
              ...
              {item.requires_inspection && (
                <div className="px-5 py-4">
                  ...
                  {/* Step 1 — Select which asset is present */}
                  <div className="mb-4">
                    ...
                    {item.assets.length === 0 ? (
                      ...
                      <select
                        value={selectedAssets[item.location_standard_id] ?? ''}
                        onChange={e => {
                          setSelectedAssets(prev => ({ ...prev, [item.location_standard_id]: e.target.value }))
                          // Clear step responses when asset changes
                          if (e.target.value) {
                            setStepResponses(prev => ({ ...prev, [e.target.value]: {} }))
                          }
                        }}
                        className={inputCls}>
                  ...
                  {/* Step 2 — Template selector (if multiple) */}
                  {selectedAsset && item.templates.length > 1 && (
                    <div className="mb-4">
                      <label className="mb-1.5 block text-sm font-medium text-zinc-700">Inspection Type</label>
                      <select
                        value={selectedTemplates[selectedAsset.id] ?? item.templates[0].id}
                        onChange={e => setSelectedTemplates(prev => ({ ...prev, [selectedAsset.id]: e.target.value }))}
                        className={inputCls}>
                  ...
                  {selectedAsset && template && (
                    <div className="flex flex-col gap-0 border border-zinc-200 rounded-lg overflow-hidden">
                      {template.steps.map((step, stepIdx) => {
                        const resp = stepResponses[selectedAsset.id]?.[step.id] ?? {}
                        return (
                          <div key={step.id} className={`px-4 py-4 ${stepIdx > 0 ? 'border-t border-zinc-100' : ''}`}>
                            ...
                            {step.step_type === 'BOOLEAN' && (
                              <div className="flex gap-3 ml-7">
                                <button
                                  onClick={() => setStepResponse(selectedAsset.id, step.id, 'boolean_value', true)}
                                ...
                                <button
                                  onClick={() => setStepResponse(selectedAsset.id, step.id, 'boolean_value', false)}
                                  className={`flex-1 rounded-lg border px-3 py-2 text-sm font-semibold transition-colors ${
                            ...
                            {step.step_type === 'NUMERIC' && (
                              <div className="ml-7">
                                <input
                                  ...
                                  onChange={e => setStepResponse(selectedAsset.id, step.id, 'numeric_value', parseFloat(e.target.value))}
                            ...
                            {step.step_type === 'TEXT' && (
                              <div className="ml-7">
                                <input
                                  ...
                                  onChange={e => setStepResponse(selectedAsset.id, step.id, 'text_value', e.target.value)}
                            ...
                            {step.step_type === 'ASSET_LINK' && (
                              <div className="ml-7">
                                ...
                                <select
                                  ...
                                  onChange={e => setStepResponse(selectedAsset.id, step.id, 'linked_asset_id', e.target.value)}
                                  className={inputCls}>
                                  <option value="">Select {step.linked_item_type_name ?? 'asset'}...</option>
                                  {step.linked_asset_options.map(a => (
                                    <option key={a.id} value={a.id}>
                                      {a.asset_tag}{a.serial_number ? ` (S/N: ${a.serial_number})` : ''}
                                    </option>
                                  ))}
