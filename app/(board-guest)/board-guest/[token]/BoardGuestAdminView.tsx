...
export default function BoardGuestAdminView({
  ...
  token: string
  state: { board: { id: string; title: string; departmentName: string | null }; label: string; lanes: Lane[]; entries: Entry[]; resources: Resource[] }
  onChange: () => void
}) {
      ...
      {lanes.map(lane => {
        ...
        return (
          <div key={lane.id} className="rounded-xl bg-white shadow-sm border border-zinc-200 p-5">
              ...
              {editingLaneId === lane.id ? (
                <div className="flex gap-2 flex-1">
                  <input
                    ...
                    autoFocus
                  />
                  <button onClick={() => handleRenameLane(lane.id)} disabled={isPending} className="text-xs font-semibold text-red-700 hover:underline">Save</button>
                  <button onClick={() => setEditingLaneId(null)} className="text-xs text-zinc-500 hover:underline">Cancel</button>
                </div>
              ) : (
                <>
                  <h3 className="text-sm font-bold text-zinc-900">{lane.name}</h3>
                  <button
              ...
              return (
                <div key={resource.id} className="rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 mb-2">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold text-zinc-800">{resource.display_desc}</p>
            ...
            {laneEntries.map(e => (
              <div key={e.id} className="flex items-center justify-between rounded-lg border border-zinc-200 px-3 py-2 mb-2">
                <p className="text-sm text-zinc-800">{e.display_name}</p>
